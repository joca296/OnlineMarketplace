...
{
    public class EfEditShippingAddressesCommand : EfCommand, IEditShippingAddressCommand
    {
        public EfEditShippingAddressesCommand(Context context) : base(context)
        {
        }
        ...
        public bool Validate(ShippingAddressDto request)
        {
            ...
            if (shippingAddresses.Where(x => x.Id == request.ShippingAddressId).Where(x => x.User.Id == request.UserId).Count() == 0)
